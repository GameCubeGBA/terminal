...
static int _GetActiveAppIconResource()
{
    ...
    if (SystemParametersInfoW(SPI_GETHIGHCONTRAST, sizeof(hcInfo), &hcInfo, 0) != 0)
    {
        if (WI_IsFlagSet(hcInfo.dwFlags, HCF_HIGHCONTRASTON))
        {
            iconResource = IDI_APPICON_HC_BLACK;

            if (GetSysColor(COLOR_WINDOW) == 0x00FFFFFF) // white window color == white high contrast
            {
...
{
    const auto iconResource{ MAKEINTRESOURCEW(_GetActiveAppIconResource()) };
    ...
    const auto hIcon{ LoadImageW(wil::GetModuleInstanceHandle(), iconResource, IMAGE_ICON, GetSystemMetrics(smXIcon), GetSystemMetrics(smYIcon), LR_SHARED) };
